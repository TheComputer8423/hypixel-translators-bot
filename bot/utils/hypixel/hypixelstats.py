...
def player_data_request(uuid, hypixel_api_key):
    try:
        player_data = requests.get(f"https://api.hypixel.net/player?key={hypixel_api_key}&uuid={uuid}").json()
        return player_data
